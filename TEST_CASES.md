...
| K-03 | `eventsAreSorted=true` with unsorted input        | Events render in wrong visual order; no developer warning   | Should log a `console.warn` or prominently document the caveat in JSDoc | Pass an unsorted `events` array with `eventsAreSorted=true`                                     |
| K-05 | `enableEnrichedEvents=true` without map           | Silently falls back to flat array; no warning emitted       | Should log a `console.warn` when `enrichedEventsByDate` is absent       | Enable `enableEnrichedEvents=true` in ControlPanel without providing `enrichedEventsByDate`     |
