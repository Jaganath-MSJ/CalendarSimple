...
- **Results file**: results are tracked in this file (PLAYWRIGHT_TEST_PLAN.md)
...
| ------ | ---- | ------------------------------------------ | ----------------------------------------------- | ------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| OPEN   | G-12 | `onMoreClick(date, hiddenEvents)`          | Second argument is always `undefined`           | Should receive `CalendarEvent[]` of hidden events | Month view + `maxEvents=2` + "Month Overflow" fixture → click "+10 more" → check console |
| OPEN   | K-03 | `eventsAreSorted=true` with unsorted input | Events render in wrong visual order; no warning | Should warn or document caveat prominently        | "Large Dataset (1k)" fixture → toggle `eventsAreSorted=true` with unordered input        |
| OPEN   | K-05 | `enableEnrichedEvents=true` without map    | Silently falls back to flat array; no warning   | Should log a warning that map is missing          | ControlPanel → Performance → `enableEnrichedEvents=true`, no `enrichedEventsByDate`      |
| OPEN   | TC3  | Negative-duration events                   | Correctly filtered (not rendered)               | `console.warn` expected in dev mode               | "Edge Cases" fixture → TC3 "Negative Duration" → check console                           |
...
> **Note**: `classNames` prop is NOT exposed in the current ControlPanel. Tests E-13–E-18 require manual prop injection via `browser_evaluate` or a future ControlPanel enhancement.
...
Sub-total **128 / 128 PASS** (TC3 documentation gap counted separately in Known Issues).
...
| TC3    | Negative duration                       | Filtered out by `useEvents` — must NOT render              | Must NOT render                                                     | Must NOT render — no dev-mode warning emitted (OPEN: add `console.warn`) | PASS ⚠ |
...
| E-13 | `classNames.root="my-cal"`                                                     | Class applied on root _(requires browser_evaluate — classNames not in ControlPanel)_ | PASS   | Class applied on root wrapper.                                                                                                                                                                                                                                                         |
...
| G-12 | `+N more` click                                                            | `onMoreClick(date, hiddenEvents)` fires; popover opens _(OPEN BUG: second arg undefined)_ | **FAIL ⚠** | `onMoreClick(date, hidden)` fires but second arg is `undefined`. Fix: `MonthEventItem.tsx` — verify the call site passes hidden events array. |
...
| H-13 | `isLoading=true` with events present (DI-3 overlay mode) | Calendar body remains visible; existing events stay on screen; overlay element present making calendar non-interactive; toggling back removes overlay cleanly | N/A    | Not run in first session — see Open Follow-up #6. When `isLoading=true` AND events are non-empty, `View.tsx` renders events under a `loadingOverlay` wrapper (background-refresh mode). |
...
| K-03 | `eventsAreSorted=true` with UNsorted input                                                                                                   | Documents the broken behavior (visual ordering wrong) — flag as known caveat _(OPEN)_    | **FAIL ⚠** | Visual ordering is wrong; library does not warn the developer. Recommend prominent JSDoc caveat.    |
...
| K-05 | `enableEnrichedEvents=true` without map                                                                                                      | Falls back gracefully or warns _(OPEN — no warning)_                                     | **FAIL ⚠** | Falls back gracefully (no crash) but emits no warning. Recommend `console.warn` when map is absent. |
...
| Date       | Branch    | Pass | Fail | N/A | Screenshots | Notes                                                                      |
| ---------- | --------- | ---- | ---- | --- | ----------- | -------------------------------------------------------------------------- |
| 2026-05-07 | version_2 | 286  | 4    | 19  | 13          | First full run. G-12/K-03/K-05/TC3 FAIL. H-13 not run (DI-3 overlay path). |
|            |           |      |      |     |             |                                                                            |
...
- The compound-component pattern (`<Calendar.Header />` + `<Calendar.View />` children) renders correctly but **bypasses** the `${testId}-container` wrapper because the `<section>` is only emitted in the no-children path (`Calendar.tsx:64`). This is by design but worth documenting for test authors.
...
| #   | Item                                                                                | Priority |
| --- | ----------------------------------------------------------------------------------- | -------- |
| 1   | Fix G-12: `onMoreClick` second arg always `undefined` — bug in `MonthEventItem.tsx` | HIGH     |
| 2   | Add `console.warn` in `useEvents` for negative-duration events (TC3)                | MED      |
| 3   | Add `console.warn` when `enableEnrichedEvents=true` but map is missing (K-05)       | MED      |
| 4   | Document `eventsAreSorted` caveat prominently in README/JSDoc (K-03)                | MED      |
| 5   | Expose `classNames` in ControlPanel → re-run E-13–E-18 interactively                | LOW      |
| 6   | Run H-13 (DI-3 overlay): events present + `isLoading=true`                          | MED      |
| 7   | Capture remaining ✗ screenshots (VIS-05, 07, 08, 10, 12–14, 17, 18b–18e)            | LOW      |
| 8   | Cross-browser sweeps (Firefox, WebKit) — stretch goal                               | LOW      |

---

## Out of Scope (Explicit)

- Adding `@playwright/test` as a permanent test suite in `package.json` — not needed; the MCP plugin handles all execution.
- Modifying any file under `src/` (library source).
- Pixel-diff visual regression baselines (we eyeball screenshots; no `toHaveScreenshot()` baseline harness).
- Drag-to-create / drag-to-resize event interactions — the library does not currently expose these as documented features.
- Mutation testing or formal coverage measurement.
- Cross-browser sweeps beyond Chromium (the MCP plugin defaults to Chromium; Firefox/WebKit recorded in Follow-up #8).
- Real backend integration / network-mocking; `events` are static fixtures.
